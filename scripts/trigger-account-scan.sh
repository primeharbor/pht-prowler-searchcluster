...
if [ -z $ACCOUNT_ID ] ; then
    echo "Must specify STACKNAME ACCOUNT_ID"
    exit 1
fi
...
{ "containerOverrides": [ { "name": "prowler", "command": ["/home/prowler/scan_account.sh", "$ACCOUNT_ID"] } ] }
EOF
...
aws ecs run-task  --cluster $CLUSTER --task-definition $TASK_ID --launch-type="FARGATE" --network-configuration file://network.json --output text --overrides file://account.json
rm network.json account.json
