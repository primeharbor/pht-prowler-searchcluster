...
if [ -z $IAM_ROLE ] ; then
    echo "Usage: $0 SEARCH_STACKNAME IAM_ROLE"
    exit 1
fi
...
echo "Using Credentials from $SECRET_ID against $ENDPOINT to add $IAM_ROLE"
...
cat ${SEARCH_STACKNAME}-mapping.json | jq '.all_access.backend_roles += ["'$IAM_ROLE'"]' | jq '.all_access | del(.hidden) | del(.reserved)' >  ${SEARCH_STACKNAME}-new-mapping.json
